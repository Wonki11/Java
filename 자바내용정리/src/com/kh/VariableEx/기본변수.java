...
변수
 ...
 자료형(참조) = Reference
 	문자열 : String // "" 사용 문자를 모두 나열하는 문자열 "" 특수문자등 모든 것 사용 가능
 	
 	상수형 : 상시적으로 변하지 않는 수 값 변화x 이름을 대문자로 표기
 	 final 자료형 상수명 = 값;
 	 final int MAX_NUM = 100; // 상시적으로 변하지 않는 수 값 변화 x 이름을 대문자로 표기
 	
 	열거형 : 상수를 하나씩 작성하기 번거로우니 관련있는 상수들을 한번에 { } 안에 작성해서
...
//변수명은 어떤 특정 행위나 행동을 하기위한 것이 아니기 때문에 메서드 안에 작성하지 않아도 괜찮음
	//sysout 와 같은 행동은 무언가를 행하기위한 표기이기 때문에 메서드 안에 작성해줘야함
public class 기본변수 {
	//필드
		//자료형 기본
			//1. 숫자
			...
			//4. true false 기다 아니다
				boolean bln = false; // boolean 은 기본값이 false 이기때문에 boolean bln = false; 는 서로 같은 뜻
